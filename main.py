...
def data_preparation(data):
    ...
    for item in data:
        ...
        if match:
            regione, carburante, servizio, prezzo = (
...
def get_from_firestore(num_of_entries, collection_name, db):
    ...
    """
    try:
        entries = db.collection(collection_name).order_by(
            "date", direction=firestore.Query.DESCENDING
        ).limit(num_of_entries).stream()
...
def data_preparation_for_forecast(docs):
    """
    Prepares the data for forecasting fuel prices.
    """
...
def forecast(prices):
    """
    Predicts the next price in a series of historical prices.
    """
    if len(prices) == 0:
        return -1
    ...
    try:
        ...
        return round(previsione.iloc[0], 2)
    except Exception as e:
